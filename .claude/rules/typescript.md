...
- **Hot-path typed-array indexing** under `noUncheckedIndexedAccess`. Math primitives in `transform/*` and the camera hot-path (`camera/common.ts` pose setters, `camera/project.ts`) read fixed, author-known indices off `Float32Array`s (`v[0]`, `m[15]`) where the flag widens the element type to `number | undefined`. These `arr[i] as number` casts are a recognised intentional-bypass *class*, justified by the hot-path failure-policy stance (see `engine-conventions.md §Failure policy` — these functions trust the caller and must not add per-call validation) plus the fact that the index is a literal the author proves in-bounds by construction. **This class does NOT require a per-site `// Boundary cast:` comment** — it is documented here once, covering all such sites. It applies only to fixed-index reads on engine-internal fixed-length typed arrays in hot-path code; a variable index from untrusted input is not covered and must be handled explicitly.
