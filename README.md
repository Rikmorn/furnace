...
bun run hello-world:dev      # browser path; open the URL it prints
bun run hello-world:dev:native  # native window path (mac/windows only)
...
- `packages/hello-world/` — reference consumer (`@furnace/hello-world`). Renders the triangle in the browser (`bun run hello-world:dev`) or in a native window (`bun run hello-world:dev:native` — wraps `furnace dev --platform=macos`).
