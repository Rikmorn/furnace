...
**Reference:** `packages/dungeon/src/world-build.ts` (gone) (`generateRegion`, `expandGridRegionFromEntry`,
`resolveSpec`), `packages/dungeon/src/bake.ts` (gone) (`bakeWorld`'s grid-region branch),
`packages/dungeon/src/world/world-loader.ts` (`assertCompatible`). The shared stamp contract itself is
`packages/dungeon/src/themes/grid-stamp.ts` (gone) (`GridStamp`). Filed 2026-07-13 from the W3 review.

## The door-width constant is duplicated in the piece box

**Context:** Surfaced in a W3 review. `packages/dungeon/src/substrate/pieces.ts` (gone)'s `PIECE_BOX`
...
REAL door width is `DOOR_W_CELLS * CELL` in `packages/dungeon/src/themes/grid-stamp.ts` (gone)
...
**Reference:** `packages/dungeon/src/substrate/pieces.ts` (gone) (`PIECE_BOX.lintel`, `PIECE_BOX.tread`),
`packages/dungeon/src/themes/grid-stamp.ts` (gone) (`DOOR_W_CELLS`, `DOOR_LANE_WIDTH`, `doorAt`).
...
`mazeRubble` layer in `packages/dungeon/src/themes/maze.ts` (gone) copy-pastes the material literal
of `DRESSING_RUBBLE` in `packages/dungeon/src/world-build.ts` (gone): same colour `[0.4, 0.38, 0.34, 1]`,
same specular `[0.03, 0.03, 0.03, 10]`, same `spacing`/`scale`/`tint`. They currently collapse
onto ONE material entry only because `instanceGroupsFromLayers` (`packages/dungeon/src/scatter.ts` (gone))
...
(`packages/dungeon/src/themes/dressing.ts` (gone)) that both `world-build.ts` and the stampers import —
...
**Reference:** `packages/dungeon/src/themes/maze.ts` (gone) (`MAZE_DRESSING_LAYERS`),
`packages/dungeon/src/world-build.ts` (gone) (`DRESSING_CRATE` / `DRESSING_RUBBLE` /
`HALL_DRESSING_LAYERS` / `SUBSTRATE_MATERIALS`), `packages/dungeon/src/scatter.ts` (gone)
...
`packages/dungeon/src/themes/maze.ts` (gone), plus an asymmetry the shared stamp contract now makes
visible.
...
**Reference:** `packages/dungeon/src/themes/maze.ts` (gone) (`maze`, `carvePlan`, `braidPass`),
`packages/dungeon/src/themes/hall.ts` (gone) (`hall` — the 28-line comparator, no door validation),
`packages/dungeon/src/themes/grid-stamp.ts` (gone) (`doorAt`'s clamp, `validateDoorApproach`).
