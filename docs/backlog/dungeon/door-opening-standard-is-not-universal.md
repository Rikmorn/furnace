...
grid door standard as **2.0 w × 3.0 h m** (`themes/grid-stamp.ts:43-44` — `DOOR_W_CELLS = 4`,
`DOOR_H_CELLS = 6`, at `CELL = 0.5`). But the ORGANIC collar still presents **2.0 × 2.8**
(`themes/cave.ts:73` — `DOOR_OPENING = { width: 2, height: 2.8 }`).
...
**Reference:** `packages/dungeon/src/themes/grid-stamp.ts` (`DOOR_W_CELLS`, `DOOR_H_CELLS`),
`packages/dungeon/src/themes/cave.ts` (`DOOR_OPENING`, applied by `mouthCollar`),
`packages/dungeon/src/built.ts` (`mouthCollar`), `docs/reference/dungeon-architecture.md`
