...
"1.6–2.6" / "0.9–1.6"), but `packages/dungeon/src/scatter.ts` consumes only `spacing.min`
...
**Reference.** `packages/dungeon/src/scatter.ts` (`spacing.min` consumption),
`packages/dungeon/src/region.ts` (`ScatterLayerSpec.spacing`), hall layers in
`packages/dungeon/src/world-build.ts` (W2 Task 13). Surfaced in the W2 execution report
