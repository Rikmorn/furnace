---
summary: per-pass GPU timing via WebGPU `timestamp-query` — query-set allocation, encoder writes, async readback — behind the `gpu.renderMs`/`computeMs` slots that already exist on the public `Snapshot`
---
