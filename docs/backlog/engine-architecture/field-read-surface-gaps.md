...
**Reference:** `packages/core/src/field/reachability.ts` (`climbNeighbours`, `fallTargets`, `markUnreachable`, `detectPits`); measured numbers in `packages/core/src/field/analyze-budget.test.ts` (`[f4-budget]` lines).
