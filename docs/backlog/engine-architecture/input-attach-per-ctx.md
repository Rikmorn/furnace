---
summary: the input module is a load-time singleton while the rest of the engine is ctx-scoped, so input emissions never reach per-ctx stats and multi-canvas / HMR mounting has no clean story
---
