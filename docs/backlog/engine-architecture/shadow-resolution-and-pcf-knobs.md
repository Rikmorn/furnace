---
summary: shadow-map resolution and the PCF tap pattern are both fixed engine constants, so a consumer cannot trade sharpness for memory or softness for cost
---
...
coverage — note CSM is the other answer there, see `cascaded-and-point-shadows.md`),
OR a consumer wanting softer shadows (wider PCF) or cheaper ones
...
`cascaded-and-point-shadows.md` (CSM is the answer for large-frustum
