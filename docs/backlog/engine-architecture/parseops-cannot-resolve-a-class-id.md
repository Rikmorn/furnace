---
summary: parse checks that a class id is storable but not that it names a class in this world's table, because `parseOps` takes no `MaterialTable` — so an unresolvable id still surfaces late at mesh time as `classOf`'s throw
---
...
`packages/core/src/field/artifact.ts` (**the oplog wire codec and `parseOps` live there —
there is no `field/oplog.ts`, and a since-deleted tracker line named one until the T5 branch
review, 2026-08-11**); `assertOpStructure` / `assertOpValid` /
