---
summary: `_nextContextId` wraps `0xffff` back to 1 with no guard and no aliasing detection, so two contexts 65535 apart are indistinguishable to every handle lookup — a stale handle then resolves into a live slot silently
---
