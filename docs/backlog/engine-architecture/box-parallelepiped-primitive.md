---
summary: axis-aligned boxes are served by `geometry.cube` + `setScale`, which needs the deferred inverse-transpose normal fix under non-uniform dims; a dedicated `geometry.box` would bake correct per-face normals and a parallelepiped would add shear
---
