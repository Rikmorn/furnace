---
summary: material, post and the shadow caster each build pipelines behind their own cache; the third site landed with an ad-hoc once-cache, so a shared refcounted `_pipelineCache` waits on a fourth
---
