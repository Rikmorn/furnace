---
summary: `lightingHelpers` composes `shadowHelpers`, so any custom shader taking the public Blinn-Phong toolkit transitively declares the shadow `@group(0)` bindings and must be created with `usesShadows`
---
