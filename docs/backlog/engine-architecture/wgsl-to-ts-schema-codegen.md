---
summary: E-B's `L` layout schema is hand-written beside the WGSL struct and the two can silently drift; E-C is the harness-side naga generator plus the build-time gate that would make the WGSL the single source of truth
---
