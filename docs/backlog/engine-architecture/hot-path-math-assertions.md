---
summary: `vec3`/`quat`/`mat4` trust the caller by failure policy, so a NaN entering a transform chain surfaces visually much later with nothing linking back to the bad input; glam-rs ships the opt-in-assert pattern this would copy
---
