---
summary: `field/types.ts` accumulates type surface across every field concern, and the undo cluster earmarked for the T3 transaction seam has no successor destination now that the seam was dropped
---
...
- `packages/core/src/field/types.ts`;
  `field-ops-wants-a-four-concern-split.md`.
