---
summary: `@furnace/core/input` carries absolute pointer position only, so the dungeon's mouselook controller reaches past the engine to `requestPointerLock` and raw `movementX`/`movementY`
---
