---
summary: `frame/render.ts` sits well past the clean-code watch line mixing ctx-bound state lifecycle with scene-pass orchestration; the preferred extraction is a `frame/render-internals.ts`, bundled with the next render-path change
---
