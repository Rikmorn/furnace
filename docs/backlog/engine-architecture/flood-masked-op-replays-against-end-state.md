---
summary: reconfigure culls replay to an affected chunk set, which is exact only while a replayed op reads inside its own bounds — a flood selection mask is unbounded by construction and traverses chunks that were never rewound
---
