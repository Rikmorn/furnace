...
**Trigger to revisit:** a scene-authoring surface returning (`editor-and-tooling/scene-chrome-returns-as-consumer-surface.md` — the inspector is the consumer this hurts), **or** the next component/resource that needs a union/exclusivity/cross-field constraint (whichever comes first). Bundle the decision with that work; don't solve it speculatively.
