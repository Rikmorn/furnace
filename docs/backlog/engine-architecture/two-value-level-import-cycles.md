---
summary: core carries two value-level (not type-only) import cycles, and the architecture test checks tier direction and surface membership but not acyclicity, so a third would land silently
---
