---
summary: a consumer who calls the public `stats.markFrameBoundary` while also using `frame.loop` gets double boundary signals; the behaviour is documented but not engine-enforced, with three candidate resolutions
---
