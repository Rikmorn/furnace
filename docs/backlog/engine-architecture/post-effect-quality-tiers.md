---
summary: no story yet for shipping more than one implementation of the same effect — quality tiers, feature-detection fallbacks, shader permutations and dynamic degradation, with the held-back ACES and AgX operators as the first seeds
---
...
**Where it sits (ruled at T5, 2026-08-11).** This is the post-effect *library's* own shape
question — how one effect ships more than one implementation and picks between them — and it
sits **below** the render-graph tier (`render-graph.md`), which is fenced above the shipped
linear chain. It is not a render-graph concern and does not wait on one.
