---
summary: RM-4 deleted `resources.list` and `resources.snapshot` from the public surface for having zero call sites; both are a ~30 LOC restore over the retained `_iterateLive` internal, with no design questions open
---
