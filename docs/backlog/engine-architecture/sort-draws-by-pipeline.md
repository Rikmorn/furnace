---
summary: `frame.render` calls `setPipeline` per mesh in submission order, so pipeline switches are unminimised — and any future sort must stay inside the opaque/blended partition F2b pinned
---
