...
Consequence for the Slice 3.1 bake path (`packages/dungeon/src/bake.ts` `appendPiece`): `RegionData.materials` carry both `color` and `specular`, but the baked scene doc can only serialize `color`. `specular` is dropped, so a loaded/baked region's lit material renders with `specular` at its zero-initialised GPU default (matte) — a visible fidelity difference from the live-generated region, which shows the theme's specular highlight. (The retired 3.0 wing baker already dropped it the same way, so this is a pre-existing convention, not a new regression; the live bakers are `packages/dungeon/scripts/bake-default-world.ts` and `bake-region.ts`.)

## Trigger to revisit
When baked regions must render pixel-identically to their live-generated preview (e.g. the cockpit's "the baked world looks like what I curated" acceptance bar tightens), or when any consumer needs per-material specular through the scene loader. Fix is a **core change**: add an optional `specular: t.color().optional()` to the `standard` material's `params` strictObject and thread it through the build (`binding.set(ctx, b, { color, specular })`), then have `bake.ts appendPiece` emit `params: { color, specular }`.
...
- `packages/dungeon/src/bake.ts` — `appendPiece` (drops specular; comment points here).
