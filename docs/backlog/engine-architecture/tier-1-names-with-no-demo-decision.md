---
summary: ten Tier-1 core names — the physics character-controller family, contact events, the shader composition fragments — sit in neither their module's `Demoed in cookbook` list nor its `Reference-only` register, so their no-demo state is unrecorded rather than decided
---
...
- Sibling debt of the same shape, filed earlier:
  `docs/backlog/engine-architecture/cookbook-demo-for-diagnostics.md` and
  `docs/backlog/engine-architecture/cookbook-normalcolor-to-lit-sweep.md` — all three are
  debt against the one-demo-per-Tier-1-feature convention.
