...
**Related, already handled:** pit flags must never be passed through `markUnreachable` — its flood cannot enter a pit by construction, so it would demote every one. Documented at both call sites (`analyze.ts` TSDoc, `core-modules.md` §field); no code enforces it, because the two live in separate return values.
