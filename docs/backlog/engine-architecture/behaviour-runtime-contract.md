---
summary: the behaviour half of the old component/behaviour design — `state`, the full lifecycle, and `requires`/typed refs validated once at the load boundary — survives the deletion of `@furnace/core/scene` with no entity concept left to attach to
---
...
> `ecs-soa-storage.md` is still live and still the storage half.
...
Still holds: SoA (`ecs-soa-storage.md`) requires knowing component shape; a
...
went); `ecs-soa-storage.md` (the deferred storage half);
