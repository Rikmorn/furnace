---
summary: `reconfigureGenerator`'s `@throws` names two failure classes and only the validation one is pinned; the empty-evaluation leg is reachable by the same one-line `evaluate`-swap fixture, and now by an agent through `session.confirm`
---
