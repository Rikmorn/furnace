---
summary: opt-in parent/child and world-matrix composition helpers over `mesh` and `transform` — never an auto-traversed framework, and the absent world-bounds layer that shadow-frustum auto-fit depends on
---
...
**Known downstream want.** `shadow-frustum-auto-fit.md` cites this entry as the missing
