---
summary: the chain evaluator builds a fresh `@group(0)` bind group per pass per frame, so the allocation count grows linearly with pass depth; pool-stable texture views would let a `WeakMap` cache it
---
