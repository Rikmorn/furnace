---
summary: the render-graph tier fenced above the shipped linear post chain — pass culling, transient-memory aliasing, cross-frame history, scene-side G-buffer inputs and compute passes between fragment passes
---
...
**Relationship to `multi-pass-post-effects.md`:** Stage 2's T2 **resolves** that entry's
...
**Reference:** Prior art: Filament FrameGraph, Bevy render graph. Distinct from
`node-graph-post-editor.md` (a consumer-facing *tool*, not a runtime).
