---
summary: the depth pass re-renders every caster every frame even for a static scene under a static light; a per-slot dirty hash would skip it without promoting `Light` out of plain per-frame value data
---
