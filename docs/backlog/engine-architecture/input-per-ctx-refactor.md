...
**Trigger to revisit:** When per-emitter input event counts become diagnostically useful (debugging input event storms, event-handler perf), OR when multi-canvas / HMR-friendly engine teardown becomes a real requirement, OR when `engine-cascade-teardown.md` lands (input would self-register via `gpu.onDispose`).
