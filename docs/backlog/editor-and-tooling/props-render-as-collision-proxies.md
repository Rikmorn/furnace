---
summary: the editor draws each placed prop as a unit primitive sized from the catalog's COLLISION block, so count, pose, scale and rough volume are right and the silhouette is not — mesh-accurate props need an `.fmesh` transport the host does not have
---
