# FieldPanel.tsx is the dig loop's single orchestrator (787 lines)
...
own files — so what is left is not markup but ORCHESTRATION: eight `host.subscribe*` effects,
...
`statsEqual`), sixteen `useState` slots mirroring host state, and the footer meter that reads
several of them.
