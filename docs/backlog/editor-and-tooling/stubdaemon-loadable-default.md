---
summary: `world-drawer.test.tsx`'s `stubDaemon` answers `field.load` with a body that does not decode unless a case opts into `loadable`, so any assertion added AFTER the load passes or fails for the wrong reason; 32 call sites make the flip a test-semantics change
---
