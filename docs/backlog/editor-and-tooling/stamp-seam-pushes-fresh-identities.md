---
summary: `subscribeStamp` publishes `structuredClone(stamp)` on every notify, so `session.params` is a new identity on every push and every downstream memo recomputes — measured at 40 `SchemaForm` renders per 20 pushes against a counterfactual 21
---
