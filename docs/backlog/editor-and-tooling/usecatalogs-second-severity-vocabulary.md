---
summary: `useCatalogs` carries its own two-member `Report` severity beside the store's four, and its `post` is a hand-written else-branch — the first catalog outcome that wants `warn` gets silently downgraded to `info`
---
...
This is the likely home of the "second caller" that `notify-severity-has-no-warn-member.md` (gone)
