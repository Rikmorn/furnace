---
summary: generation preview takes over the Viewport panel via a visibility swap, because a `display:none` canvas is zero-size and `bindToCanvas` throws — dockview backgrounds inactive tabs the same way, so a real fix is a WebGPU-canvas-lifecycle mini-slice, not an added panel
---
