---
summary: `rebuildProps` tears the placed-prop layer down and rebuilds it on every path that could have changed it, and re-creates the three unit proxy geometries each time — one fix with a cache half and a change-detection half
---
...
(`props-ignore-the-slice-plane.md`), whose most likely implementation
...
`props-ignore-the-slice-plane.md` (the sibling that would make this a hot path).
