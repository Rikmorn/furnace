---
summary: `remeshOne` wraps the worker round-trip and `applyMesh` in one `try` written for transient per-chunk failures, so a totally broken material cache leaves an empty viewport, a green suite and one `console.warn` per chunk
---
