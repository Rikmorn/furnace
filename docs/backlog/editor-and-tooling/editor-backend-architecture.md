...
5. **Dual-mode, not either/or — the serialized artifact is the interchange.** Furnace is *both* a code-first library (consumers `import @furnace/core` and build a world in TS — the one-off / website-embed path) *and* an editor-authored engine (author in the editor, ship artifacts — the full-game path). These coexist because both converge on one runtime representation: the editor produces an artifact, code can produce the same artifact directly, and a consumer with only core installed loads it. This is standard engine architecture (Godot `.tscn`, Unity scenes/prefabs, three.js editor JSON + `ObjectLoader`). **(T2: the artifact is the field world — `chunks/` + `oplog.json` + the derived bake — not a scene document. `field.bakeFieldWorld` is pure and lives in core, so the editor's export, a consumer's own bake script and a headless test emit byte-identical files from the same ops. The claim is more true than when written, not less: the code-first path is now a plain script, `packages/dungeon/scripts/bake-default-world.ts`.)**
6. **The editor emits only core-loadable artifacts.** The load-bearing discipline that keeps dual-mode honest: the *loader lives in core*, and the editor adds *workflow and opinionated defaults*, never *runtime semantics core cannot reconstruct*. Defaults bake into the artifact as concrete data. Higher-level abstractions the editor leverages must live *below* it (in core or a core-adjacent opinionated layer), or artifacts won't port to plain-core projects and "both" silently collapses into editor-primary. This is also what keeps the editor frontend genuinely *thin*. (Editor-frontend UI tech is an open downstream call — likely to differ from the in-page Svelte 5 of `ui-foundation.md`, e.g. a desktop Tauri/Electron shell; it does not affect the core contract.) **(T2: this held under test. Every generator, brush, mesher and skinner the editor drives is `@furnace/core/field` surface — the editor holds none of its own, and the daemon now holds no schema knowledge at all. The one place the discipline is visibly enforced is `generation.bake`: the browser produces the bytes and the daemon only writes them, so there is no server-side authoring path that could accrue semantics core cannot reconstruct.)**
...
- `engine-architecture/unbuilt-tier-2-modules.md` §Behaviour contract / `engine-architecture/unbuilt-tier-2-modules.md` §ECS / data-oriented SoA layout — component definitions / data-oriented storage (heavier; not required to start).
...
epic's doc debt (`core-modules.md` `./scene` + editor docs). **(M7 never ran; the doc debt was
cleared at foundations T2 instead, by deleting the surface it described.)**
...
- **Cloud-native project** (PlayCanvas): project + assets live in the cloud, real-time multi-user, no local repo. The only *true* "cloud version" — costs local-first, contradicts furnace's dual-mode thesis (your repo, your git). Possible optional hosted offering later atop the same artifact format; never the default.
