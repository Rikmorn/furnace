---
summary: `beginMove` takes an `entityId` and `pointerPress` arms a drag only on an already-selected COMMITTED entity, so a live CREATE session's region moves by arrow keys or the card's d-pad — the gap is press arbitration, not difficulty
---

# A CREATE session's ghost cannot be dragged — only a committed entity can

*(Absorbed into the M5B viewport register at T5, 2026-08-11 — filed long after M5B but against the same surface.)*
...
`box-selection-is-two-clicks.md`, since both are the same question about what a press
