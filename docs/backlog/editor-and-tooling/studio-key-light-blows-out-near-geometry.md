---
summary: rock within a metre or two of the eye reads white under the studio rig — a tuning note on a feature that passed its gate, deliberately left for the user live in front of the viewport, and now agent-visible through `viewport.capture` without being agent-fixable
---
...
stop-condition answer if studio shading had read FLAT, and it did not, so it was never filed
as a deferral. It is recorded as `cavity-ao-matcap-shading-fallback.md` (filed at
genre-contracts as the record of a fallback that was never scheduled, which is why this
sentence used to say no entry existed). Its trigger still stands — this one is about a rig
that works being too bright up close.
...
- `editor-viewport-is-non-hdr.md` — the other deferral on this same viewport render path.
  Both were merged into one tracker at T5 for that reason; whoever takes either should read
  the other, because a mode that renders the game's post chain changes what "too bright"
  even means here.
