...
**Reference:** `packages/editor/src/frontend/inspector/` (SchemaForm, field renderers — NumberField, ColorField, kind→renderer registry), the host preview path (`previewEntity`/`revertEntity`, transform fast-path), the dockview chrome (`packages/editor/src/frontend/`), `docs/reference/editor-architecture.md`, sibling entries: `editor-M5B-viewport-interaction.md` §6 (resource cascade), `editor-seams-and-preview-deferrals.md` § *Editor viewport HDR context + post-chain preview*, `editor-chrome-authoring-gaps.md` § *Editor authoring of the `textures` + `effects` resource tables*, `../engine-architecture/scene-schema-cannot-encode-cross-field-constraints.md`.
