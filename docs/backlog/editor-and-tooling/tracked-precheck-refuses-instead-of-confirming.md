---
summary: `saveWorld`'s tracked pre-check REFUSES the save when `world.list` fails; escalating into the overwrite confirm with the uncertainty named strictly dominates both shipped alternatives, but costs a fourth `SaveOutcome` status, its own routing branch, copy and tests
---
