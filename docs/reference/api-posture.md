...
- **Async vs sync factories.** A factory is `async` (returns `Promise<T>`) **iff it compiles a pipeline or shader** (`shader.create`/`load`/`unlit`/`normalColor`, `material.create`, `post.create`, `material.createPipeline`). Pure-data/buffer factories are synchronous (`mesh.create`, `geometry.create`/`cube`/`plane`, `camera.perspective`/`orthographic`).
