...
| Kind | Members |
|---|---|
| **Resource** | `Mesh`, `Material<L>` (phantom `L` carries the binding layout; defaults wide), `Geometry`, `Effect<L>` (phantom `L`, same as `Material<L>`), `Shader<L>`, `Binding<L>`; the re-exported `*Handle` aliases (`MeshHandle`, `MaterialHandle`, `GeometryHandle`, `EffectHandle`, `ShaderHandle`, `BindingHandle`), `AnyResourceHandle`, `ResourceKind` |
...
| **Lifecycle-op** | `gpu.dispose`; `resources.disposeAll`; `shader.destroy`; `binding.destroy`; `mesh.destroy`; `geometry.destroy`; `material.destroy`; `post.destroy`; `input.attach`/`detach`; `frame.loop`/`fixedLoop` (+ `stop`/`pause`/`resume`); all `on*` subscriptions (`gpu.onResize`/`onDeviceLost`/`onUncapturedError`, `input.on*`, `stats.onFrame`); `camera.bindToCanvas`; `Emitter.on`/`clear`; `Measurement.end` |
