...
1. **Higher abstractions build *on top*, not *into*.** Scene graphs, ECS, retained render lists, render graphs — these are higher-tier conveniences. They belong in their own modules layered over the core surface; they do **not** alter the low-level vocabulary. A proposed new concept that is a higher-tier convenience goes to its own module (or a triggered backlog entry), never folded into the core resource/value vocabulary. Building those modules *from* the public core surface is also a forcing function: if a higher-tier convenience can't be composed cleanly from the handle vocabulary, the primitives are missing something — treat that friction as a primitive-gap signal, not a reason to reach into internals. `@furnace/core/field` is the live instance of this rule; `core/scene` was the other and was deleted in foundations T2 (2026-08-05) when its last consumer went — which is the rule working, not failing: a Tier-2 convenience that stops being used is *removable* precisely because nothing below it ever depended on it.
...
- **R4 — Verb family per kind.** Pick the verb from the concept's kind:
  ...
  - **The two compose.** A command that is both an immediate primitive AND retargeted reads `draw<Primitive>To<Target>` — `frame.drawLinesToTexture` (foundations T4c). This bullet is a *naming* rule; it does not by itself decide separate-command-vs-target-field. **The one case where that IS decided: when the on-screen and retargeted forms would carry different failure-policy stances, they must be separate exports** — R9 assigns a stance per export, so one name cannot hold two. That is what forced the split here (`drawLines` is runtime-quiet about the one frame it cannot serve; `drawLinesToTexture` owns no frame and is setup-loud like `renderToTexture`), and it is the only part of the case that generalises. When the stances match, the shape is an open question argued at the code — see `docs/backlog/engine-architecture/frame-surface-gaps.md` §`render` / `renderToTexture` unified pass-target union, which already weighed a unified `{ target }` union for the managed pair and rejected it on R8 tier grounds, not on posture grounds.
...
- **R5 — Input-bundle naming.** `*Descriptor` = the full specification passed to a low-level `create()` for a custom resource (`MaterialDescriptor`, `EffectDescriptor`). `*Options` = a (usually defaulted) config bundle for a factory / command / setup function (`RenderOptions`, `LoopOptions`, …). `*Data` = raw buffer contents, not a config spec (`GeometryData`). Sibling descriptors that share fields extract a common **internal** base type (e.g. the render commands share an internal `RenderPassBase`); the named public types remain the surface.
  ...
  **Exception — strict superset (2026-08-09, foundations T4c).** The base-extraction half applies to *genuine siblings*, where each bundle has fields the other lacks (`RenderOptions` has `effects`/`fog`; `RenderToTextureOptions` has `texture`/`depthTexture` — neither contains the other, hence `RenderPassBase`). When one bundle is a **strict superset** of the other, it extends the public type directly instead: `DrawLinesToTextureOptions = DrawLinesOptions & { texture; depthTexture }`. An extracted base there would add a third name that no one reads and make publicness a shape artifact rather than a decision — the churn R8's surface-membership refinement warns against. The test is "does the smaller bundle exist as public surface in its own right?" — for `DrawLinesOptions` it does.
...
- **R7 — Escape-hatch convention.** Escape hatches (raw-WebGPU access, the manual instrumentation path that bypasses the managed loop/render, or a construction path that bypasses the managed one) are **standalone functions, never folded into a broader interface**, and **documented as escape hatches in their TSDoc** (open the summary with `**Escape hatch.**`). They stay co-located with their managed sibling (e.g. `material.createPipeline` next to `material.create`), so choosing managed-vs-raw is a local decision. There is deliberately **no `raw.*` namespace and no registry** — that marking would be more opinionated than the tier warrants. The current set:
  ...
  | `physics.createHeadlessPhysicsContext` | a `PhysicsContext` with no GPU behind it (bypasses `gpu.requestContext`) |
...
- **Async vs sync factories.** A factory is `async` (returns `Promise<T>`) **iff it must await a one-time system init or compile a pipeline/shader** — current async factories: `shader.create`/`load`/`unlit`/`normalColor`, `material.create`, `post.create`/`post.createPasses`/`post.tonemap`/`post.bloom`, `material.createPipeline` (shader/pipeline compile; note `post.createPasses` and `post.create` are technically await-free at call time because pipelines build lazily at render, but both return `Promise<Effect>` by contract — mirroring tonemap/bloom, which ARE async, so the family is uniformly async); `physics.createWorld` (awaits Rapier's one-time wasm init via `ensureRapierInit`, memoized after first call). Pure-data/buffer factories are synchronous (`mesh.create`, `geometry.create`/`cube`/`plane`/`sphere`/`cylinder`, `camera.perspective`/`orthographic`, `physics.createBody`, `frame.fixedClock`).
...
| **Value-type** | `Camera`, `Context`; the `transform` types (`Vec2`/`Vec3`/`Vec4`/`Quat`/`Mat3`/`Mat4`) + `CameraMatrices`; `FitPolicy`/`Anchor`/`OrthographicBounds`/`ScreenProjection`; event records (`FrameInfo`/`ResizeEvent`/`KeyEvent`/`PointerEvent`/`WheelEvent`/`PointerSnapshot`); `Snapshot`/`Measurement`/`Path`/`PathValue`; `LogLevel`/`LogEntry`/`LogSink` + `consoleSink`; `FrameLoopHandle`/`Emitter`; the error classes (`FurnaceError`/`FurnaceGpuError`/`FurnaceInputError`); `KeyCode`/`PointerButton`/`PointerType`; `ToneMapOperator` (`"neutral" \| "reinhard"` — selects the tone-mapping operator for `post.tonemap`; a plain discriminator string, no GPU ownership); `PassInput` (`"scene" \| "prev" \| { intermediate: string }` — discriminated union selecting a pass's colour source; pure data, no GPU ownership); `PassOutput` (`{ scale?, format?, intermediate? }` — pass render-target shape; pure data, no GPU ownership); `CollisionEvent` (contact begin/end record drained after a `step` — `{ a: Body; b: Body; started: boolean }`); `FixedClock` (the `fixedClock` accumulator object — methods, no lifecycle, like `FrameLoopHandle`/`Emitter`); `DebugLines` (`{ vertices: Float32Array; colors: Float32Array }` — collider wireframe line data, transient, from `physics.getDebugLines`); `SamplerParams` (sampler config — all fields optional; no public `Sampler` handle, engine-cached and deduped per descriptor — see the no-public-Sampler-handle decision in this table); `ProceduralResult` (`{ data: Uint8Array; width: number; height: number }` — pure pixel data, feed to `texture.create`); `ShaderSource` (`{ body, deps }` — composable WGSL fragment, immutable value, no handle/lifecycle/`destroy`, distinct from the compiled `Shader` resource; factory `shader.source`, flattened by `toWgsl`, fed to `shader.create`. Closest industry terms: three.js `ShaderChunk`, naga_oil composable module, Metal `MTLLibrary`/`MTLFunction` — no AAA source-level type-of-art exists). The public engine lighting/shadow fragments `shader.sceneBinding` / `shader.lightingHelpers` / `shader.shadowHelpers` are pre-built `ShaderSource` data values (no handle/lifecycle) — composed into custom lit/shadow-receiving shaders with `shader.create(ctx, src, { usesScene: true, usesShadows: true })`; `lightingHelpers` depends on `sceneBinding` and composes `shadowHelpers`, which itself depends on `sceneBinding` (so composing `lightingHelpers` pulls in both). `shadowHelpers` (Stage 4) classifies identically to `lightingHelpers`/`sceneBinding` — a DATA-kind `ShaderSource`, no new resource/handle/lifecycle. `Light` (discriminated-union value-type — `directional`/`point`/`spot`, no handle/lifecycle, matching the `Camera` posture) and `Ambient` (hemisphere ambient value-type) are per-frame data passed via `RenderOptions.lights`/`ambient` — and, since foundations T4c, the identically-named `RenderToTextureOptions` fields; the engine packs them into the Scene UBO declared by `shader.sceneBinding`. The optional `Light.shadow` config (`DirectionalShadow`/`SpotShadow`, Stage 4) is a **per-frame value-DATA extension** of `Light` — a pure-data config nested in the value-type, presence = casts; it introduces **no new resource kind, handle, or lifecycle** (the shadow maps are engine-owned, not consumer-handled), consistent with `Light` being plain value data. (R-conformance: the Stage 4 surface adds no new function shape — `Light.shadow` is value-DATA, `shadowHelpers` is DATA-kind `ShaderSource`, and `usesShadows` is a `ShaderCreateOpts` flag mirroring `usesScene` — so R1–R9 are unaffected; no new verb, no new handle, no escape-hatch.) |
| **Descriptor** | `MaterialDescriptor<L>` (gains `binding?: Binding<L>` — the typed `@group(1)` data path; raw `bindings?` retained for advanced use; gains `texture?: { texture: Texture; sampler?: SamplerParams }` — the `@group(1)` texture path, mutually exclusive with `binding`/`bindings`), `EffectDescriptor<L>` (`shader: Shader<L>` + the same typed `binding?: Binding<L>` / raw `bindings?` `@group(1)` paths as material) (create-specs); `PassesDescriptor` (`{ passes: PassDescriptor[] }` — the ordered-pass list passed to `post.createPasses`); `PassDescriptor` (`{ shader, inputs, output?, binding?, bindings?, blend? }` — one pass in a `createPasses` chain; the per-pass counterpart of `EffectDescriptor`); `RequestContextOptions` (config — gains `sampleCount?: 1 \| 4` and `hdr?: boolean` in Stage 2a: `sampleCount` selects the MSAA pass count, `hdr` selects the working color format — ctx-level value-config on an otherwise plain options bundle) / `LoopOptions`/`RenderOptions`/`RenderToTextureOptions`/`PerspectiveOptions`/`OrthographicOptions`/`DrawLinesOptions`/`DrawLinesToTextureOptions` (config; `RenderOptions` gained `lights?: Light[]` + `ambient?: Ambient` in Stage 3 Phase 2 and `RenderToTextureOptions` gained the same pair in foundations T4c — per-frame Value-type data fields the engine packs into the Scene UBO, not GPU-owned descriptors; `DrawLinesToTextureOptions` is `DrawLinesOptions` plus the two consumer-owned attachments, extended directly rather than through an extracted base because one is a strict superset of the other — R5's base-extraction advice is for genuine siblings like `render`/`renderToTexture`, which share `RenderPassBase`); `GeometryData` (raw data); `TextureDescriptor` (discriminated union — `{ data, width, height, colorSpace?, mipmaps? }` or `{ source: ImageBitmap, colorSpace?, mipmaps? }` — passed to `texture.create`); `RigidMeshDescriptor` (`{ body: BodyDescriptor; mesh: { geometry, material } }` — the `rigidMesh.create` create-spec); `WorldDescriptor` (`{ gravity: Vec3Tuple; lengthUnit?: number }` — `physics.createWorld` create-spec); `BodyDescriptor` (type/shape/position/rotation/velocities/density + the four pass-through material scalars `friction`/`restitution`/`linearDamping`/`angularDamping` — `physics.createBody` create-spec); `ShapeDescriptor` (`{ ball: number } | { cuboid: Vec3Tuple } | { cylinder: { halfHeight: number; radius: number } }` — collider shape used inside `BodyDescriptor`). Note: `Shader` has no public descriptor — a `ShaderSource` (value-type) or a bare WGSL string is passed directly to `shader.create` (`shader.load` is string-only). |
| **Factory** | `gpu.requestContext`; `shader.create`/`load` (async — compile from WGSL source / fetch + compile; accept optional `opts?: ShaderCreateOpts<L>` to declare the `@group(1)` layout schema plus the `usesScene` / `usesShadows` per-frame-binding flags, still classified Factory) / `shader.unlit`/`shader.lit`/`shader.normalColor`/`shader.textured`/`shader.texturedLit` (async — engine-owned built-in shaders, shared per ctx; `textured`/`texturedLit` carry `textureBinding: true` instead of a uniform layout; `lit`/`texturedLit` set `usesScene: true` + `usesShadows: true` internally so they receive shadows transparently) / `shader._layoutOf` (internal accessor, see `@furnace/core/shader` Internal table in `core-modules.md`); `binding.create` (sync — allocates a `GPUBuffer` + CPU scratch from a shader or explicit layout; see `@furnace/core/binding`); `texture.create` (async — uploads rgba8 data or `ImageBitmap` as a GPU texture; setup-loud) / `texture.load` (async — fetch → decode → `create`; setup-loud); `texture.checkerboard` / `texture.uvGrid` (sync, **pure** — generate rgba8 pixel buffers, no GPU involvement; classified Factory because they produce the data bundle that feeds `texture.create`); `material.create`; `mesh.create`; `geometry.create`/`cube`/`plane`/`sphere`/`cylinder`; `post.create` / `post.createPasses` (async — **[PROVISIONAL name for `createPasses` — see `docs/backlog/engine-architecture/public-api-naming-audit.md`]** multi-pass chain factory; each `PassDescriptor` becomes one fullscreen pass in a pool-backed linear chain) / `post.tonemap` (async — built-in HDR→LDR tone-map effect factory; engine-owned tonemap shader cached + shared per ctx, same pattern as `shader.lit`; `post.destroy` releases the effect pipeline ref and frees the owned `@group(1)` binding) / `post.bloom` (async — built-in COD/Jimenez dual-filter bloom, assembled on `createPasses`; requires `hdr: true`; `post.destroy` frees the owned shared params binding); `camera.perspective`/`orthographic`; `rigidMesh.create` (sync — builds + owns a `Body` + a `Mesh`, seeds the interpolation buffers); `events.createEmitter`; `stats.snapshot`/`startMeasurement`; `physics.createWorld` (async — awaits Rapier wasm init, then allocates backend world; cold-path throws on non-finite gravity); `physics.createBody` (sync — allocates body + collider in an existing world; cold-path throws on bad descriptor or stale world handle); `frame.fixedClock` (sync — ctx-less fixed-step accumulator; cold-path throws on non-positive `fixedDtMs` / non-integer `maxCatchupTicks`) |
...
| **Command** | `frame.render`, `frame.renderToTexture`, `frame.drawLines`, `frame.drawLinesToTexture` |
| **Sugar-helper** | `camera.policy.{stretch,preserveHeight,preserveWidth}`; `material.blend.{straightAlpha,premultiplied,additive}` |
| **Escape-hatch** | `material.createPipeline`; `frame.encode`; `gpu.getCurrentTextureView`; `stats.recordDraw`; `stats.markFrameBoundary`; `physics.createHeadlessPhysicsContext` (see R7 table) |
