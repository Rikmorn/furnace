...
| Kind | Members |
|---|---|
| **Resource** | `Mesh`, `Material<L>` (phantom `L` carries the binding layout; defaults wide), `Geometry`, `Effect`, `Shader<L>`, `Binding<L>`; the re-exported `*Handle` aliases (`MeshHandle`, `MaterialHandle`, `GeometryHandle`, `EffectHandle`, `ShaderHandle`, `BindingHandle`), `AnyResourceHandle`, `ResourceKind` |
