...
- **Async vs sync factories.** A factory is `async` (returns `Promise<T>`) **iff it compiles a pipeline or shader** (`shader.create`/`load`/`unlit`/`normalColor`, `material.create`, `post.create`, `material.createPipeline`). Pure-data/buffer factories are synchronous (`mesh.create`, `geometry.create`/`cube`/`plane`/`sphere`, `camera.perspective`/`orthographic`).
...
| Kind | Members |
|---|---|
| **Resource** | `Mesh`, `Material<L>` (phantom `L` carries the binding layout; defaults wide), `Geometry`, `Effect<L>` (phantom `L`, same as `Material<L>`), `Shader<L>`, `Binding<L>`, `RigidMesh` (the `@furnace/core/rigid-mesh` composite — owns a `Body` + a `Mesh`); the re-exported `*Handle` aliases (`MeshHandle`, `MaterialHandle`, `GeometryHandle`, `EffectHandle`, `ShaderHandle`, `BindingHandle`), `AnyResourceHandle`, `ResourceKind` |
