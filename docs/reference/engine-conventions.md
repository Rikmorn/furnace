...
**Material-less shaders are supported.** A shader may declare no `@group(1)` (e.g. `normalColor`, or a custom camera+object-only shader). Since the per-draw object lives at `@group(2)`, `@group(1)` then becomes an empty *intermediate* bind-group slot — and WebGPU rejects a draw that leaves an intermediate slot unbound below a bound higher slot. `frame.render` handles this transparently by binding an empty bind group at `@group(1)` when the material has none, so material-less shaders Just Work. (See `docs/learnings/2026-06-07-webgpu-empty-intermediate-bind-group.md`.)
