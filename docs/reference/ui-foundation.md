...
When the first concrete in-scene UI need surfaces, see `docs/backlog/editor-and-tooling/in-scene-ui-primitive-for-occluded-cases.md` and start from the reverted commit.
...
- **Rust-side UI rendering (egui via wasm, Vello).** No silver bullet given Furnace's device-sharing constraint: `navigator.gpu` lives in JS-land and a Rust+wgpu wasm module would request a second `GPUDevice` that cannot directly share buffers or textures with the JS-side device. Vello also explicitly does not target the web today. Tracked as an emerging-tech watch (see `docs/backlog/editor-and-tooling/emerging-tech-watch-html-in-canvas-vello.md`) alongside the WICG "HTML in Canvas" proposal.
...
- `docs/backlog/editor-and-tooling/emerging-tech-watch-html-in-canvas-vello.md` — WICG HTML-in-Canvas / Vello browser readiness.
