...
| `drawLines` | `(ctx: Context, opts: DrawLinesOptions) => void` | Immediate `line-list` overlay (Command). Draws `opts.vertices` (flat xyz line-list) colored per-vertex by `opts.colors` (RGBA), transformed by `opts.camera`, as a second pass composited over the current frame against the scene depth with no depth write. **MSAA-aware (F2b):** on a `sampleCount: 1` context it targets the swap-chain view directly (`loadOp:"load"`); on a `sampleCount: 4` context it renders into the stored MSAA scene color target and resolves to the swap chain (the effects-less scene pass stores its MSAA color + depth for exactly this — pre-F2b this combination built an invalid pass and every line overlay was silently dropped, see `docs/learnings/2026-07-21-invisible-line-overlays.md`). **Documented limitation:** MSAA + a non-empty post chain + `drawLines` is unsupported — the resolve would clobber post output — and is a once-per-context warn + skip, never frame corruption. Depth mode is set by `opts.occlude` (default `true`): `true` → `depthCompare:"less-equal"` (occluded behind nearer meshes — physics wireframes, AABB highlights); `false` → `depthCompare:"always"` (always-on-top — gizmos). Call after `frame.render` in the same frame. Pipelines (per depth mode, at the context's multisample count) + grow-on-demand buffers are engine-owned per context. Warm-path-validate: throws on disposed ctx / null camera / null arrays; empty `vertices` → no-op. For the off-screen target instead of the swap chain, see `drawLinesToTexture` (a separate command, because its failure posture is setup-loud rather than runtime-quiet). |
| `renderToTexture` | `(ctx: Context, opts: RenderToTextureOptions) => void` | Like `render`, but the color target is a consumer-supplied `GPUTexture`. Draws `opts.meshes` then each `opts.instanced` group (one instanced draw per group, after `meshes`) into the off-screen target. No post-effects chain. **Lit off-screen passes are supported:** `opts.lights` / `opts.ambient` are threaded into the same Scene UBO packer `render` uses, so an off-screen pass can reproduce the on-screen composition (thumbnail bakes, planar reflections, agent-facing viewport captures); omit both for the ambient-only default. **Shadows are still not cast off-screen** — the call supplies no shadow casters, so `_packScene` leaves every light's Scene-UBO shadow slot at its `-1` default and the receiver's `fr_shadowFactor` returns 1.0 *without sampling* the shadow array; a light's `shadow` config is inert here. The array is bound but never read, and is **not** cleared by this call (only `frame.render`'s shadow passes write it), so during a capture it still holds the previous on-screen frame's maps — harmless only because the slot is `-1`. `fog` is not accepted (the Scene UBO's fog lane is always packed disabled, density `0`). Depth presence must agree with each drawn material's depth state; a mismatch (or a color-target format ≠ the working color format, or a supplied `depthTexture` format ≠ `depth24plus`, or an MSAA context) throws `FurnaceGpuError`. |
| `drawLinesToTexture` | `(ctx: Context, opts: DrawLinesToTextureOptions) => void` | Off-screen variant of `drawLines` (Command) — composites a `line-list` over a consumer-supplied `GPUTexture` + depth texture instead of the swap chain, `loadOp:"load"` on both, so repeated calls compose. The intended partner of `renderToTexture`: that writes the meshes, this adds the overlays, and the caller reads back one texture holding both (the editor's agent-facing viewport capture). `opts.occlude` behaves exactly as on-screen. **Depth is required** (it is optional on `renderToTexture`) — both line pipelines declare a `depth24plus` depth-stencil state, so WebGPU demands a matching attachment even with `occlude:false`. **Setup-loud, unlike `drawLines`:** each checked precondition throws `FurnaceGpuError` at call time — disposed ctx, null camera/arrays, missing `texture`/`depthTexture`, an MSAA context, an **HDR context** (the line pipelines write `ctx.format` while `renderToTexture` writes `workingColorFormat`; on HDR those differ, so no one texture can hold both), `texture.format ≠ ctx.format`, `depthTexture.format ≠ depth24plus`, and either attachment's `sampleCount ≠ 1` (the ctx guard only proves the *pipelines* are single-sample; consumer textures are independent — `renderToTexture` does **not** check this, the one place the newer command is stricter). Attachment size agreement and `RENDER_ATTACHMENT` usage are deliberately left to WebGPU, whose own error names them. Empty `vertices` → no-op after validation. |
...
| `DrawLinesToTextureOptions` | `DrawLinesOptions & { texture: GPUTexture; depthTexture: GPUTexture }` | Input bundle for `drawLinesToTexture`. `texture` must be format `ctx.format` (on an LDR context that is also the working color format, so the very texture `renderToTexture` drew into is valid); `depthTexture` is **required** and must be `depth24plus` — pass the same one the `renderToTexture` call used so `occlude: true` tests against that pass's real scene depth. |
| `RenderToTextureOptions` | `{ texture: GPUTexture; meshes: Mesh[]; instanced?: InstancedMesh[]; camera: Camera; depthTexture?: GPUTexture; lights?: Light[]; ambient?: Ambient; clearColor?: Vec4; clearDepth?: number }` | `lights` / `ambient` — same per-frame Scene-UBO semantics as `RenderOptions`' (omitted/empty `lights` → ambient-only; omitted `ambient` → the neutral low default; same `MAX_LIGHTS` clamp + warn-once, never throws). There is deliberately **no `fog`** here and shadows never cast off-screen. `instanced` — optional instanced draw groups, drawn after `meshes` (one instanced draw per group) into the off-screen target; resolved against the dedicated instanced-mesh pool, same invalid-handle semantics as `meshes`. `clearColor` is a linear-space RGBA `Vec4`. Omit `depthTexture` **only if every drawn material has depth disabled** (`material.create` with `depth: false`, or a built-in factory with `depthEnabled: false`); a depth/format mismatch (or a color format ≠ the working color format, or depth format ≠ `depth24plus`) throws `FurnaceGpuError`. The working color format is `rgba16float` for HDR contexts, the swap-chain surface format for LDR. **MSAA contexts (`sampleCount: 4`) are not supported** — call `renderToTexture` only on `sampleCount: 1` contexts. |
...
- `renderToTexture`, `RenderToTextureOptions` (including `lights` / `ambient` — the `pipLit` toggle puts a `shader.lit` probe in BOTH draw lists so only the off-screen copy goes ambient-only when the lights are withheld) → `cookbook/render-target`.
...
- `drawLines`, `DrawLinesOptions` — no dedicated page; exercised incidentally by `cookbook/physics` (collider wireframes from `physics.getDebugLines`).
- `drawLinesToTexture`, `DrawLinesToTextureOptions` — no demo, and deliberately not the one to add first: its managed sibling `drawLines` has no page either, so demoing the off-screen variant would invert the teaching order. Its only consumer today is the editor's off-screen viewport capture (mesh pass + overlays into one readable texture), which is a tool-pipeline lesson rather than an engine-primitive one. **Trigger:** when `drawLines` earns a page, the off-screen target belongs on it as one control.
- `Fog` — per-frame `RenderOptions` field; no demo. The cookbook has no atmospheric-scene page for it to read on, and `cookbook/lighting`'s close-up subject would show almost no fog gradient. (Pre-existing gap, recorded here 2026-08-09 rather than left silent.)
...
| `load` | `(ctx: Context, url: string, opts?: ShaderCreateOpts<L>) => Promise<Shader<L>>` | Fetch WGSL from `url` and compile it. Accepts the same `opts` as `create`. Does **not** resolve `// @include` directives (to share snippets, compose with `shader.source`/`toWgsl` and pass the result to `create`; `// @include` string-syntax resolution stays deferred — see `include-composition-resolver.md`) and does **not** cache by URL (the browser HTTP-caches the bytes; reuse the returned handle to deduplicate). Setup-loud: throws `FurnaceError` on a non-OK HTTP response; throws `FurnaceError` if the fetched WGSL fails to compile. |
...
| `Registry` | `{ register(name, entry); get(name); entries(); reset() }` | The instance shape. `reset()` is tests-only (suites re-register after). `entries()` returns `[name, entry]` pairs in registration order (Map insertion order) as a fresh array per call — re-added in foundations T3a for the T3b ToolManager's ordered enumeration pass, the reader the T2 removal note was waiting on. |
...
- **Smart objects — reconfigure (F3a)** — `reconfigureGenerator(store, log, entityId,
  ...
  **Known gap (flood reads):** culling rewinds only the affected chunks, so every other
  ...
  Tracked in `docs/backlog/engine-architecture/flood-masked-op-replays-against-end-state.md`.
