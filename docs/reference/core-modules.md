...
- Post's internal `_pipelineCache` (a facade in `post/pipeline-cache.ts` over `acquirePostPipeline` / `releasePostPipeline` on the per-ctx `ResourceManager`), `_resolvePassPipeline` + `_effectTeardown` (in `post/effect.ts`, used by `frame/render.ts` and `post/passes.ts`), `_ensureFullscreenVS` (in `post/fullscreen.ts`), `_effectPipelineHashKey` / `_buildEffectPipelineDescriptor` (in `post/pipeline.ts`), the transient-target pool `_acquirePoolTarget` / `_releasePoolTarget` / `_poolStats` (in `post/pool.ts`), and `_ensurePostSampler` (in `post/intermediate.ts`). None re-exported from `post/index.ts`.
