...
- Shader's internal `_createShader` (in `shader/shader.ts`, used by `create`, `load`, and the built-in shader factories). The built-in shader factories (`unlit` / `lit` / `normalColor`) are now public — see the `@furnace/core/shader` Public table above.
