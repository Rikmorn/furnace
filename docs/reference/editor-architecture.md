...
**⑫ ColorField no-op-commit guard — REVERTED (was a regression).** M5B shipped a guard that compared the blur value against `rgba` before firing `onCommit`, intending to skip a no-op revision bump on focus+blur with no edit. But `rgba` is sourced from `SchemaForm`'s working draft, and `onChange → onPreview` advances that draft to the picked color *before* blur — so by blur time `rgba` already equalled the picked value and the guard suppressed **every** real commit, not just no-ops (colors silently failed to stick). A field that only ever sees `SchemaForm`'s draft structurally cannot implement this guard. `ColorField` is back to committing unconditionally on blur (the M5A behavior). The no-op-revision papercut is reopened in the backlog — its correct home is one layer up (`SchemaForm`, which holds both the live draft and the committed baseline) or the document-session mutation layer.
