...
test("mixed → indeterminate hides the check glyph and shows the minus glyph", () => {
  ...
  expect(container.querySelector("svg.lucide-check")?.getAttribute("class")).toContain(
    "hidden",
  );
  expect(
...
test("checked → shows the check glyph and hides the minus glyph", () => {
  ...
  expect(container.querySelector("svg.lucide-minus")?.getAttribute("class")).toContain(
    "hidden",
  );
});
