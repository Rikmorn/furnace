...
// `tool-params.tsx`, `status-keymap.ts`, `lib/actions.ts`) and `tool-registry.ts` (since
// T3c) — so a core value-import added to ANY of them lands in the main bundle exactly like a
// direct one, and the chrome's own import (`../../shared/catalog.ts`) matches none of the
// three specifier rules below, so nothing else would catch it. There is no "host-only" module
// left down here to make an exception of; assume there never is. (Earlier revisions of this
// note carried a running count of which modules the chrome had reached yet — first four of
// five, then five of six — and both were stale within a tranche. The scan walks the DIRECTORY
// rather than a list for exactly that reason, so the rule has never depended on the tally
// being right. The names above are a reader's aid; the walk is the guarantee.) This scan is
// what keeps the "engine-free" half of the `shared/` rule a fact rather than a comment. (Its
// React-free half is enforced by this file's mirror, `no-chrome-leakage.test.ts`, which keeps
// the chrome out of the layers below it the way this one keeps the engine out of the chrome.)
