						...
						<div className="flex flex-col gap-3">
							{settingsSelected ? (
								...
								<p className="text-sm text-muted-foreground">
									Nothing selected. Pick an entity, or “World” for scene
									settings.
								</p>
							)}
							{doc && reflection && (
	...
	const rows = Object.entries(resources ?? {}).flatMap(([table, entries]) =>
		Object.keys(entries ?? {})
			...
			.map((id) => ({
				table,
				id,
				entry: (entries as Record<string, unknown>)[id],
			})),
	);
				...
				return (
					...
					>
						<SchemaForm
							...
							onCancel={() =>
								selected.forEach((e) => actions.revertEntity(e.id))
							}
						/>
					</CollapsibleSection>
				);
