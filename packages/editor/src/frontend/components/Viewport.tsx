...
export function Viewport() {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: state.doc is a deliberate re-run trigger — re-seed the triad pose from the host on every document change (open/switch/revision); the body reads the pose from the host, not from state.doc
	useEffect(() => {
		const host = hostRef.current;
		if (!host || state.status !== "ready") return;
