...
export function TopBar() {
	return (
		<header className="flex h-10 shrink-0 items-center gap-2 border-b border-border bg-card px-2">
			<BurgerMenu />
			{/* MIGRATION (until F4.5b): a placeholder chip. The world name + dirty dot come
          from the lifted world state; today the field host holds an unnamed world
          until the toolbar's Save gives it one. */}
			<span className="flex items-center gap-1.5 rounded-sm bg-muted px-2 py-0.5 text-xs text-muted-foreground">
				untitled
			</span>
			<div className="flex-1" />
			{/* MIGRATION (until F4.5b): inert until the bake flow moves out of the field
          toolbar. Disabled rather than dead — see BurgerMenu. */}
			<Button type="button" size="sm" variant="secondary" disabled>
				Bake
			</Button>
			{/* MIGRATION (until F4.5b): the hint is ahead of its binding — ⌘\ lands with
          the palette layer that has something to hide. */}
