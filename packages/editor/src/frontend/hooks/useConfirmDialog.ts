...
/**
 * The in-chrome confirm-dialog state machine (extracted from App, Task 11 —
 * replaces `window.confirm`). `confirmRef` mirrors the pending request synchronously
 * so BOTH: (a) `openConfirm` refuses to clobber an already-pending prompt, and the
 * global keydown listener can suppress every binding while one is open (a second
 * prompt would strand the first's onCancel — e.g. the discard-scene prompt's
 * refreshSession that clears `loading`); and (b) `resolveConfirm` fires each request's
 * callback exactly once — the guard also absorbs a rapid double-click that lands while
 * Radix's exit-animation `Presence` still has the dialog (and its buttons) mounted.
 *
