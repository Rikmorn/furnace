...
- **Generation cockpit**: an HDR preview host (`bloom→tonemap` + headlamp + orbit; **fog is a viewport VIEW-FLAG, default off**; both canvases stay laid out — visibility swap, never `display:none`); the **World panel** (3.3 W3 — was the Generation panel; panel id `generation` persists in layouts) + ephemeral App-owned session; generation runs in a module Web Worker (runId-disciplined protocol; **cancel = terminate + lazy respawn, instant mid-attempt**; the worker is never respawned alone on `bundle-outdated` — a page reload refreshes worker + main thread together). Since W3 the panel ASSEMBLES a world draft (attach-on-add tree, `world-draft.ts` — connectors own doors; per-region reroll; leaf-only remove; full-spec preview snapshot), drives the WORLD flow (`runWorld`/`bakeWorld`; deterministic, no attempts machinery), and can retarget `worlds/index.json` at bake ("make this the game's world" — a second cleanDir-free upload); `generation.bake` writes browser-uploaded file sets root-contained (optional `cleanDir`), emitting `generation-baked` SSE.
