...
- **Field authoring (One Field F1)**: the **Field panel** + `FieldHost` (a PreviewHost-class host) — dig a `@furnace/core/field` chunked voxel world with fly camera, chunk-keyed undo/redo, flat-shaded/headlamp shading, background remeshing in a dedicated engine-direct worker (transferables, dirty-set, ~1.1 ms/16³ chunk on the M1); saves/bakes ride `generation.bake`, loading via the `field.load` daemon command; world name is never prefilled. All three hosts defer init to the first nonzero canvas measure (`lib/init-when-sized.ts`). As-built: `editor-architecture.md` §15.
