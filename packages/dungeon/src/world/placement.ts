...
// `placePiece` (transform a whole region).
import { mat4, quat, vec3 } from "@furnace/core/transform";
import { transformAabb } from "./aabb.ts";
import type {
  Connection,
  InstanceData,
  RegionCollider,
 ...
 *  Extracted so a caller can transform ONE local portal to world without re-placing a
 *  whole region, and reused inside {@link placePiece} so the two share exactly one
 *  rotate+translate. */
