...
## Current state (Epic 3 · 3.3 Worlds — W3 landed 2026-07-13: THE PHASE BAR)

Epics 1–2 CLOSED; Epic 3 recharted 2026-07-11 as **3.3 Worlds**. W3 sealed the charter's phase gate: a world assembled from scratch in the editor's World panel, baked, made the game's default, and walked in game rendering. W4 (the clean-cut sweep) remains before 3.3 closes.
...
- **Legacy wing pipeline (until the W4 sweep, unshipped)**: `topology.ts` (graph generation; `WorldNode.themeParams` = the provenance contract) → the `layout.ts` placer (collision-aware free-space embedding, `LayoutBudget`-bounded + `deadlineMs` wall-clock fail-fast; `COCKPIT_CONFIG`/`COCKPIT_BUDGET`/`COCKPIT_ENVELOPE` — the measured 2–12 envelope) → `themes/` + `connect.ts` + `built.ts` + `scatter.ts` → `region.ts` `RegionData` → `realize.ts`; `world.ts worldAttempts()` owns retry policy + `seed:k` derivation; `bakeWing`/`wing-loader.ts` under `regions/<name>/`.
...
Then **3.4 Seeing** → **3.5 Curating** (renumbered from 3.2.4/3.2.5 — they run AFTER 3.3, against the world model) → **3.6 JIT regions** (backlogged: `docs/backlog/dungeon/jit-runtime-regions.md`). The old deep-gen/streaming/LLM epic remains **Epic 4**, unchanged.
