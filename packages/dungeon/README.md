...
- **F3b the cave & the entities** — **SEALED 2026-07-25** (core cave + scatter generators, placement ops + artifact; this package's catalog + derived-collider half; the editor's scatter authoring, void cast, and segment brush) → **F4** seeing (next) → **F5** scale → **F6** clean cut + seal; the accumulated gate-UX sets batch into a dedicated UX/polish stage after the feature phases
