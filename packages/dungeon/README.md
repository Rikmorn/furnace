...
- **Placements / props (One Field F3b, gate pending)**: an optional manifest `placements: "placements.json"` adds the entity half — `buildPlacementInstances` resolves each group's archetype in `catalog/entities.json` (setup-loud if absent), splits records by `variantIndex`, and uploads one instanced mesh per (archetype, variant) via `field.packPlacementMatrices`, joining the field world's instanced draw list beside the F2a kit. Colliders are DERIVED at load and never serialized (`createPlacementColliders` → one static body per record, shaped by the catalog collision primitive: box scaled per-axis, sphere/capsule by max scale axis — exact for scatter's uniform-scale records). A props-free world loads exactly as before. **Known gap**: colliders are centred on the scatter surface-projection point, so realistically sized rocks are STEPPED OVER rather than blocking (measured: climbed ≤~0.56 m, blocks ≥~0.77 m) — `docs/backlog/dungeon/placement-props-stepped-over.md`, deferred to F4. As-built: `dungeon-architecture.md` §8.
...
- **F3b the cave & the entities** — landed, **gate pending** (core cave + scatter generators, placement ops + artifact; this package's catalog + derived-collider half; the editor's scatter authoring, void cast, and segment brush) → **F4** seeing → **F5** scale → **F6** clean cut + seal
