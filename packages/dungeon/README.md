...
- **Worlds (3.3, go-forward)**: `world-spec.ts` (`WorldSpec` — declarative regions with class/algorithm/params/seed/placement, connectors, `startRegion`; connectivity validation) → `world-build.ts realizeWorldSpec` (deterministic, search-free — join-derived placements) → `connector.ts organicTunnel` (the connector OWNS its volume; `TUNNEL_RADIUS` must match the cave bore) → `bake.ts bakeWorld` → `worlds/<name>/` (ONE merged `world.scene.json` + `.fmesh` sidecars + `world.json` manifest written LAST — crash-safety; provenance per region AND connector; voxels never serialize, proxies re-expand).
