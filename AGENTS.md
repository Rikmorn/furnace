...
- `packages/hello-world/` (`@furnace/hello-world`, private) — the reference consumer. Renders the WebGPU triangle with the Svelte 5 FPS overlay. Owns its own `index.html`, `bunfig.toml`, `serve.ts`, and dev-server choice. Imports core via `@furnace/core` (workspace symlink). Uses `furnace dev --platform=macos` for native dev (via `bun run hello-world:dev:native`) — dogfooding the consumer experience.
- `packages/dungeon/` (`@furnace/dungeon`, private) — the first-person dungeon-crawler demo, the engine's go-forward consumer app (vision — an atmospheric, eventually LLM-streamed crawler: `~/.claude/.../memory/project_dungeon_crawler_vision.md`). Browser-first; imports core via the workspace symlink; owns `index.html` + `serve.ts`; `bun run dungeon:dev`; editor-openable (`bun run edit` in the package). **As-built architecture (canonical current-state): `docs/reference/dungeon-architecture.md`** — game loop, traversal/collision, the generator library, the bake/load pipeline, invariants, testing posture. Chronological slice seals: `docs/learnings/seal-log.md`. Current state in one breath (Epics 1–2 CLOSED; Epic 3 recharted 2026-07-11 as **3.3 Worlds**, W1 landed): the GAME boots the baked default WORLD — `world-loader.ts` reads `worlds/index.json` → the world's manifest → ONE merged `world.scene.json`, re-expands proxies/dressing deterministically, spawns at the manifest `playerStart` (setup-loud if missing; `worlds/default/` ships as committed fixtures; hand level + wing path RETIRED from `main.ts`, their modules in-tree until the W4 sweep); the player is a Rapier capsule driven by the custom `CharacterMover` (collide-and-slide on core casts; generated/organic geometry collides against field-derived VOXEL PROXIES — the confirmed ghost-free bridge, Jolt endgame in backlog); HDR `bloom→tonemap` + fog + torch + instanced scatter dressing. The GENERATOR is a pure library the cockpit consumes: `topology.ts` (graph generation; **`WorldNode.themeParams` records each node's exact extra generator params — the bake/load provenance contract**) → the `layout.ts` placer (deterministic collision-aware incremental embedding, `LayoutBudget`-bounded, fail-fast) → `themes/` + `connect.ts` + `built.ts` + `scatter.ts` → the `region.ts` `RegionData` contract → `realize.ts`; `world.ts` **`worldAttempts()`** iterator owns ALL retry policy and `seed:k` derivation (`buildWorld` drains it; the cockpit drains it in a generation Web Worker — terminate = instant cancel; `COCKPIT_CONFIG`/`COCKPIT_BUDGET`/`COCKPIT_ENVELOPE` — the measured 2–12 envelope table; `LayoutBudget.deadlineMs` = search-time-only wall-clock fail-fast, and `bakeWing` STRIPS it: bake replay is counted-only deterministic). The BAKE pipeline (3.3 W1): `bake.ts bakeWorld(spec, name?)` → ONE merged render-only `world.scene.json` + `.fmesh` sidecars + a `world.json` manifest written LAST (crash-safety; provenance per region AND connector — voxels never serialize, proxies re-expand) under `worlds/<name>/`; **the BROWSER bakes and uploads** (JSC vs V8 diverge on placement transcendentals — NEVER regenerate placement cross-engine: `docs/learnings/2026-07-06-cross-engine-placement-determinism.md`; committed fixtures are sound — placement BAKES, only local-frame ops re-expand), the daemon validates + writes; `world-loader.ts` re-expands proxies + dressing deterministically at load, guarded placement-level. The LEGACY wing pipeline (`bakeWing`/`wing-loader.ts`, `regions/<name>/`) remains in-tree, unshipped, until the W4 sweep. **Epic 3 doctrine:** editor-time generation may use search-class algorithms — a human with reroll, caps, and curation tools absorbs failure; runtime generation is restricted to construction-guaranteed or degrade-never-fail vocabularies via generator entities, and the guarantee class is an explicit setup-loud field on the socket contract. Ladder (recharted 2026-07-11): 3.0–3.2.3 ✅ → **3.3 Worlds** — worlds = graphs of REGIONS (per-class native interior algorithms: grid-built on the validated two-resolution voxel substrate, field-organic) + CONNECTORS ("open a passage", implemented per class-pair; substrate evidence: `docs/research/2026-07-11-voxel-substrate-spike-findings.md`): **W1 world model ✅** (field-only, user-gated 2026-07-11) → W2 substrate + grid-built halls → W3 maze + World-panel assembly (= the phase gate) → W4 clean-cut sweep (mesh generators + free-space placer + mesh connector kit delete) → then **3.4 Seeing** → **3.5 Curating** (renumbered from 3.2.4/3.2.5 — they run AFTER 3.3, against the world model) → **3.6 JIT regions** (backlogged: `docs/backlog/dungeon/jit-runtime-regions.md`). The old deep-gen/streaming/LLM epic remains **Epic 4**, unchanged.
...
- **Sealing a slice/epic?** APPEND the dated seal paragraph to `docs/learnings/seal-log.md` and update the relevant `docs/reference/*-architecture.md` (the canonical current-state). Do NOT add slice status/detail to this file — AGENTS.md is operational guidance (setup, commands, foundational rules, pointers), not a status board; its package bullets change only when something an agent *operationally needs* changed (a command, a package's role or structure, a setup step, a foundational invariant). The bullets went through a 50 KB→summary compaction on 2026-07-06 precisely because seals accreted here; don't regrow them — if the detail matters, it belongs in `docs/reference/` and the bullet already links there.
