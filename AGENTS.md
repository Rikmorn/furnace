...
- `packages/editor/` (`@furnace/editor`, private) — the editor (M3, in progress). A Node-portable daemon (no `Bun.*` in `src/`, enforced by `tests/no-bun-leakage.test.ts`) that serves the editor frontend same-origin, exposes a zod-validated command registry over `POST /api/<command>` (the M4 MCP seam), and esbuild-bundles the consumer's `@furnace/core` + extensions + viewport-host from the consumer's own `node_modules` (project-first resolution — the editor contains no engine). React chrome lands in M3 Plan B. Design: `docs/backlog/editor-and-tooling/editor-backend-architecture.md`.
