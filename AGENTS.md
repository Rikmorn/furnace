...
**The browser dev servers serve a startup-cached `@furnace/core`.** All three (`hello-world:dev`, `dungeon:dev`, `cookbook:dev`) run `bun --hot serve.ts`, which re-bundles the entry package's own files on navigation but serves a bundle of the `@furnace/core` workspace dependency captured at server startup — the workspace symlink resolves at bundle time. **Edited anything under `packages/core/src/`? Restart the server** before trusting a visual checkpoint; re-navigating or hard-reloading is not enough, and the tab keeps running the old core. Editing only the entry package's own files (`packages/dungeon/src/`, `packages/hello-world/src/`) needs no restart — navigation re-bundles those. When a visual result contradicts a green `bun test` plus a clean source review, suspect the stale bundle before the new code: tests run live `src`, the browser runs the startup-cached bundle. The incident behind the rule: `docs/learnings/2026-06-15-dev-server-stale-core-bundle.md`.
...
**Both test modes run the same population, and that is the point.** `bun run test` (4 workers, each test file in a fresh realm) and the serial `bun test` report the same case count, the same pass count and the same one capability skip; the parallel lane is a fraction of the serial wall clock. **Serial remains the close/review standard** — run it before sealing a slice or opening a review, and if the two modes ever disagree, that disagreement is itself the bug. The worker count is a ruled policy about this machine rather than a default to inherit: bun's default of one worker per core saturates the laptop, and cases then fail on contention alone — a load-dependent set that reaches files carrying no budget, failing as assertions against a ceiling rather than as blown timeouts. Ruling, evidence and reopening trigger: `docs/reference/test-gate.md`.

## Agent skills & `.claude` structure

- **Root `.claude/` holds only truly shared config and skills** (currently `playwright-cli`, `teach`, `tranche-review`). Package-specific skills live in that package's own `.claude/skills/` — today: `packages/editor/.claude/skills/{impeccable, shadcn, vercel-react-best-practices, vercel-react-view-transitions, sculpting-worlds}` (the four third-party ones because the editor is the repo's only React surface; `sculpting-worlds` is first-party — the agent world-building skill).
...
- **Sealing a slice/epic?** ADD a dated seal file under `docs/learnings/seals/` carrying `summary:`/`sealed:`/`seq:` frontmatter, then run `bun run docs:index` — the index row is GENERATED, never hand-written (following `docs/learnings/seals/README.md` §Writing a seal for what that file must answer), update the relevant `docs/reference/*-architecture.md` (the canonical current-state), and update the affected **package README** (`packages/<pkg>/README.md` owns that package's detail + current-state summary). **Do NOT touch AGENTS.md at seals — ever.** AGENTS.md is operational guidance (setup, commands, foundational rules, one-line package pointers), not a status board; its package lines change only when a package is added/removed/renamed or its one-line role changes. This rule was restated 2026-07-11 after seal edits crept back twice ("operational clause" is not a loophole): package detail lives in the READMEs, status lives in seals + reference docs.
