...
- `packages/editor/` (`@furnace/editor`, private) — the editor (M4 command layer landed). A Node-portable daemon (no `Bun.*` in `src/`, enforced by `tests/no-bun-leakage.test.ts`) that serves the editor frontend same-origin and owns a zod-validated command registry over `POST /api/<command>`: a mutable document session (open/save, transactional registry-validated mutations, snapshot undo/redo), an SSE change feed (`GET /api/events`), and scene-file watching with conflict semantics. It esbuild-bundles the consumer's `@furnace/core` + extensions twice — a browser engine bundle and a node-platform registry bundle for daemon-side validation (project-first resolution — the editor contains no engine). React 19 + dockview chrome (prebuilt to `dist/frontend`, served by the daemon); dogfood: `bun run edit` in `packages/hello-world`. AI bindings (MCP mount, viewport capture, embedded/outbound LLM) are deferred to `docs/backlog/editor-and-tooling/editor-ai-integration-milestone.md`. Architecture: `docs/reference/editor-architecture.md`.
